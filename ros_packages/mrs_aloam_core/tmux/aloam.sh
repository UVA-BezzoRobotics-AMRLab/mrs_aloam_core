...
pre_input="export UAV_NAME="${UAV_NAME}"; export UAV_NUMBER=${UAV_NUMBER}; export RUN_TYPE=simulation; export UAV_TYPE=x500; export WORLD_NAME=simulation; export SENSORS='garmin_down, ouster'; export ODOMETRY_TYPE='liosam'"
...
"
  'Control' "waitForHw; roslaunch mrs_uav_core core.launch platform_config:=`rospack find mrs_uav_gazebo_simulation`/config/mrs_uav_system/$UAV_TYPE.yaml custom_config:=`rospack find mrs_aloam_estimator_plugin`/custom_configs/mrs_uav_managers.yaml world_config:=./custom_configs/world_config.yaml network_config:=./custom_configs/network_config.yaml
"
  'SLAM' 'waitForHw; roslaunch mrs_aloam_core slam_pipeline.launch OUSTER_TYPE:=OS0-32
'
...
"
  'rviz' "waitForControl; waitForControl; roslaunch mrs_aloam_core rviz.launch
"
